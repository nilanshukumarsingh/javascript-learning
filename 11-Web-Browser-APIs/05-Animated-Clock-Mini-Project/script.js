...
function clock() {
  ...
  ctx.save(); // save everything before default state
  ctx.beginPath(); // 
  ctx.lineWidth = 14;
  // ctx.strokeStyle = '#800000';
  ctx.strokeStyle = borderColor.value;
  ctx.fillStyle = faceColor.value;
  ctx.arc(0, 0, 142, 0, Math.PI * 2, true); // Draw a circle location (x, y) radius startAngle endAngle counterClockWise
  ctx.stroke();
  ctx.fill();
  ctx.restore();
  ...
  // Get current time
  const hr = now.getHours() % 12; // It will give 24 hours
